...
prop_scalarProd :: ( InnerSpace v, Num (Scalar v), CheckableVect (Scalar v)
                   , IsList v, Item v ~ Scalar v )
                       => v -> v -> Similarity
prop_scalarProd v w = v<.>w ≈ sum (zipWith (*) (toList v) (toList w))
...
tests = testGroup "Vector-space identities"
  [testGroup "(checked by QuickCheck)"
     ...
     , testSimilarity "scalarProd @ℝⁿ" 0
        (prop_scalarProd :: FinSuppSeq ℝ ℝ -> FinSuppSeq ℝ ℝ -> Similarity)
     , testSimilarity "distrib @ℝ⁴ⁿ" 1e-14
        (prop_distrib :: ℝ -> FinSuppSeq ℝX4 ℝ -> FinSuppSeq ℝX4 ℝ -> Similarity)
     , testSimilarity "scalarProd @ℝ⁴ⁿ" 0
        (prop_scalarProd :: FinSuppSeq ℝX4 ℝ -> FinSuppSeq ℝX4 ℝ -> Similarity)
     ]
  ]
